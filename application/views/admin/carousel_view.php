<?php	
	$editurl = base_url()."admin/carousel/editedcarousel";
	$createurl = base_url()."admin/carousel/createcarousel";
?>
<div>
	<a href = "" class = "btn btn-large btn-info addcarousel" onclick="$('#carouselmodal').modal('show');return false;">Create carousel</a>
</div>
...
var strHtml = '<span class = "carousel-addData"><div class="control-group">'			
			...
			+'<label class="control-label" for="focusedInput">Carousel Image</label>'
			+'<div class="controls">';
	if(isedit != undefined)
	{
	...
	strHtml	+='<input class="input-large focused carousel_image" id="focusedInput" name="carousel_image_'+count+'" type="file" >'
			...
			+'</div>'			
			+'<div class="control-group">'
			+'<label class="control-label" for="focusedInput">Carousel Caption</label>'
			+'<div class="controls">'
	...
	function()
	{
		...
		(
			...
			function()
			{
				...
				$(".modal-header h3").html("Add new carousel");		
		...
		(
			{
				...
				{	
					...
					if(response.status == "success")
					{
						...
						$(".modal-header h3").html("Edit Carousel");
						//carousel_name  carousel_image display_status
