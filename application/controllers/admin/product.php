...
class Product extends SecureAccess 
{
	...
	private function listProduct()
	{
		...
		(
			...
			"link"=>array(base_url()."admin/product/getproduct/%@$%",base_url()."admin/product/deleteproduct/%@$%"),
			"clickable"=>array("","")
		);
		$label = "Product List";
	...
	{		
		if(isset($_GET) && isset($_GET["q"]))
		{
			...
			{
				$where .= " AND 
				find_in_set(".$query["brand"].",product.brands_id)" ;
			}
		}
		else
		{
		...
		/*	$products = array();
			...
			$titles = array("Halal Kart Products");
			$filename = "Halal Kart - Products Excel Sheet";
			$pages = 1;
			$dates = array();
	...
	{
		echo json_encode(array("status"=>"success","data"=>$this->product_model->getProduct($product_id)));
	}
	...
	public function deleteproduct($product_id)
	{
		...
		$this->product_model->deleteProduct($product_id,$deleteUser);
...
//		echo "<pre>";print_r($this->posteddata);die;
	...
	{		
		...
		{//echo "<pre>";print_r($this->posteddata);die;
			...
			for($i=1; $i<=$this->posteddata['product_count']; $i++)			
			{
				...
				$arrRetval[$cnt]['product_ingredients'] = $this->posteddata["product_ingredients_".$i];
				$arrRetval[$cnt]['product_price'] = $this->posteddata["product_price_".$i];
				...
				$arrRetval[$cnt]['brands_id'] = implode(",",$this->posteddata["brands_id_".$i]);
	...
	private function processProduct($is_csv = FALSE)
	{
		...
		$this->brandsData =  $this->brands_model->getBrands("","",1);	
		if($is_csv)
		$this->getNamefromIds($this->brandsData,"brand_id","brand_name","brands_name","brands_id",",");
		else
		$this->getNamefromIds($this->brandsData,"brand_id","brand_name","brands_name","brands_id",",<br /><br />");
